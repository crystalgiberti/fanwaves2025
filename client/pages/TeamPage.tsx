...
export default function TeamPage() {
  ...
  if (!team) {
    return <Layout><div>Team not found</div></Layout>;
  }
        ...
        <div className="container relative mx-auto px-4">
          ...
          <div className="max-w-4xl">
            ...
            <p className="text-xl text-white/80 max-w-2xl">
              Official {team.city} {team.name} fan gear and merchandise. Show your team pride with authentic apparel and accessories.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground mt-4 max-w-2xl mx-auto">
              Browse our complete collection of {team.city} {team.name} merchandise
            </p>
          </div>
              ...
              >
                <Card className="group cursor-pointer hover:shadow-lg transition-all duration-300 border-2 hover:border-primary/20">
                  <CardHeader className="text-center pb-4">
                    ...
                    >
                      {category.id === 'hats' && <Grid3X3 className="h-8 w-8" />}
                      {category.id === 'shirts' && <ShoppingBag className="h-8 w-8" />}
                      {category.id === 'jerseys' && <Star className="h-8 w-8" />}
                      {category.id === 'accessories' && <Grid3X3 className="h-8 w-8" />}
                      {category.id === 'ladies' && <ShoppingBag className="h-8 w-8" />}
                      {category.id === 'pants' && <Grid3X3 className="h-8 w-8" />}
                      {category.id === 'outerwear' && <ShoppingBag className="h-8 w-8" />}
                    </div>
