...
import { X, MessageCircle, Send, Mic, MicOff, TrendingUp, Star, ShoppingBag } from 'lucide-react';
...
export default function RiffChatbot() {
  ...
  const riffPersonality = {
    ...
    traits: ["OG sports commentator", "very cool", "very funny", "approachable", "up-to-the-minute sports news", "NFL & NCAA trending fashion expert"],
    greeting: "Yo! I'm Riff, your sports commentator and Fan Waves guide! 🏈 Ready to find some legendary gear? I've got the inside scoop on all the hottest trends and can help you navigate the site like a pro!"
  };
  ...
  const riffResponses = {
    ...
    custom: "Now THIS is where the magic happens! ✨ Our custom gear designer is next level - you can create jerseys, hats, accessories with ANY team colors and fonts. Want me to walk you through it? It's like having your own personal design studio!",
    trending: "Yo, let me drop some knowledge! 📈 Right now it's all about vintage-style snapbacks, oversized jerseys for that throwback vibe, and custom fan chains are HUGE! Fans are mixing classic with modern - it's beautiful!",
    ...
    teams: "We've got ALL the teams! 💯 Every NFL squad, major NCAA programs - if you bleed team colors, we got the gear to match. Drop me a team name and I'll show you what's hot!",
  ...
  const generateRiffResponse = (message: string) => {
    ...
    if (message.includes('trend') || message.includes('hot') || message.includes('popular')) {
      return {
        ...
        <Card className="fixed bottom-4 right-4 z-50 w-[calc(100vw-32px)] max-w-md h-[70vh] max-h-[600px] shadow-2xl border-0 bg-white/95 backdrop-blur-sm md:w-96">
          ...
          <div className="p-4 border-t bg-white">
            ...
            <div className="mt-3 flex space-x-2 overflow-x-auto pb-1">
              <Button variant="outline" size="sm" onClick={() => handleSendMessage("What's trending?")}>
                <TrendingUp className="w-3 h-3 mr-1" />
                Trending
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleSendMessage("Help me find gear")}>
                <ShoppingBag className="w-3 h-3 mr-1" />
                Find Gear
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleSendMessage("Custom options")}>
                <Star className="w-3 h-3 mr-1" />
                Custom
              </Button>
            </div>
